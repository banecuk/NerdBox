...
void ProcessesScreen::createWidgets() {
    ...
    widgetManager_.addWidget(std::unique_ptr<ProcessListWidget>(
        new ProcessListWidget(uiController_->getDisplayContext(),
                              WidgetInterface::Dimensions{0, 0, Layout::kScreenW, 272}, 500,
                              processData_)));
    ...
    widgetManager_.addWidget(std::unique_ptr<ButtonWidget>(new ButtonWidget(
        ...
        WidgetInterface::Dimensions{0, Layout::kBottomBarY, Layout::kButtonSize,
                                    Layout::kButtonSize},
        0, EventType::SHOW_CPU_CLOCK, [this](EventType action) { this->handleAction(action); },
        TFT_BLACK, TFT_WHITE)));
