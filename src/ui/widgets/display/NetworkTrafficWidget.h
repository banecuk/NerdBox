...
// speed (Environment.h's ETH_UPLOAD_MBPS / ETH_DOWNLOAD_MBPS) — grey when
// idle, green/yellow/orange as the link fills up, red once at or over the
// configured capacity.
//
...
class NetworkTrafficWidget : public Widget {
 ...
 private:
    ...
    // Colour by utilisation percentage of the configured link speed.
    static uint16_t trafficColor(float percent);
